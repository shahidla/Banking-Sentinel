# Banking Sentinel — Project Context v6
## Definitive Document — For Claude Code / VS Code
## This is the ONLY file Claude Code needs. All previous versions superseded.
## Last updated: May 2026 — Complete rewrite incorporating all decisions through full conversation
...
System fetches from live official APRA URLs:
  ...
  apra.gov.au/industries/1/standards
        ↓
Documents chunked, embedded via Generative AI Hub
...
This is not a simulation. Real documents from APRA's live website applied in real time.
...
This must also be REAL — the architecture must genuinely support zero code change document ingestion that immediately affects assessments.
...
**Regulatory update flow (Twinkle 2 — live APRA connection):**
```
"Sync Latest APRA Standards" button clicked in UI
        ↓
CAP action fetchApraDocuments() called
Fetches from live APRA URLs:
  handbook.apra.gov.au/standard/aps-221
  apra.gov.au/.../CPS-230-clean.pdf
        ↓
Documents chunked and embedded via RPT/Claude embedding
Stored in HANA Vector (RegulatoryDocuments entity)
        ↓
CPI publishes regulatory_update event to Solace
        ↓
Banking Sentinel marks all cached assessments as stale
        ↓
"Re-running B-001 with updated regulations..."
Synthesis Agent retrieves new document via hana_vector_search
New guideline applied immediately
Zero code change — document fetch was the only action
```
...
| Graph Traversal | SAP HANA Cloud Knowledge Graph Engine | HANA Cloud trial | Same | SAP AI Golden Path recommended. AI-native graph reasoning. Multi-hop SPARQL traversal on BUT050 and BCA_GUARANTOR. GA since Q1 2025. Enable via HANA Cloud Central. |
...
| Tabular Risk Scoring | RPT-1 (HANA Cloud SQL stored procedure) | HANA Cloud trial — RPT-1 available via AI Core and HANA SQL | Same | Foundation model for tabular prediction. No training. API call to SAP Generative AI Hub. Score borrower risk from features table. Faster and cheaper than LLM for structured data. |
...
| Evaluation | RAGAS (Python) | pip install ragas — runs as separate Python evaluation script, NOT inside the TypeScript application | Same | RAG quality scored automatically. 20-question evaluation dataset. Python only — no npm package exists. |
...
# Evaluation
# Python evaluation scripts (run separately from TypeScript application)
pip install ragas            # RAG evaluation — Python only, no npm package
pip install langchain openai # RAGAS dependencies
pip install hana-ml          # HANA PAL Python client — optional, PAL also callable via SQLScript from CAP
```
...
**Python** — LangGraph TypeScript (@langchain/langgraph) used throughout. SAP Cloud SDK for AI officially supports TypeScript. Python used ONLY for RAGAS evaluation scripts. This is a confirmed decision — TypeScript for the application, Python for evaluation.

**SQLite** — NOT used for Banking Sentinel. SQLite cannot support HANA Knowledge Graph Engine (SPARQL), HANA Vector Engine, HANA PAL, or RPT-1 API integration. HANA Cloud is used from Phase 0 — day one. No local SQLite fallback.
...
**Critical: Not all five agents always run — conditional routing based on risk signal:**

After Pattern Agent completes, a risk score is available from RPT-1:
- **Low risk (score < 30):** Skip Relationship and Trajectory agents entirely. Route directly to Synthesis. Synthesis generates "Low risk — no connected party concerns, no regulatory breaches, no action required." Cost saving, speed improvement.
- **High risk (score ≥ 30):** Continue to Relationship Agent → Trajectory Agent → Self-RAG → Human Approval → Synthesis. Full five-agent sequential pipeline.

This is not a shortcut — it is the correct architectural decision. Running graph traversal on every low-risk borrower wastes compute and obscures the signal. Only high-risk customers need the full relational reasoning chain.

```typescript
// Add to graph topology
graph.addConditionalEdges("pattern", routeAfterPattern, {
  "low_risk": "synthesis",        // score < 30 — skip graph traversal
  "high_risk": "relationship"     // score >= 30 — full pipeline
})
```

**UI behaviour for low risk:**
Panel 2 shows Relationship and Trajectory nodes as greyed out with label "Not required — low risk signal." This makes the conditional routing visible and explainable to the audience.
...
Job: Traverse HANA Knowledge Graph Engine to find connected parties using SPARQL. Reason about the NATURE and STRENGTH of each connection — not just that one exists. Parent-subsidiary carries full exposure consolidation. Common shareholding carries partial. Family trust requires reasoning not a rule.
...
const hanaGraphTool = {
  ...
  description: "Traverse TRBK entity relationships via HANA Knowledge Graph Engine",
  inputSchema: {
...
Example popup when Relationship Agent activates:
```
RELATIONSHIP AGENT ACTIVATED

Purpose: Traverse connected party network to find hidden relationships

Why triggered: Pattern Agent detected HIGH risk signal (score 74, confidence 0.89)

Technology:
  — HANA Knowledge Graph Engine
  — SPARQL query on BUT050 + BCA_GUARANTOR tables
  — Up to 8 hops deep

Looking for:
  — Guarantor connections (BCA_GUARANTOR)
  — Parent entity relationships (BUT050)
  — Group exposure calculation vs APS 221 limit

[Click OK to run this agent]
```

User clicks OK. Agent runs. Next agent activates. Next popup appears.

**Demo Mode (toggle: OFF)**
No popups. Full pipeline runs without interruption. Clean, fast, impressive for technical audiences who already understand the architecture. Ideal for conference presentations and video recording.
...
One button. Two completely different audience experiences. No code change between modes — just a UI flag that controls popup rendering.

**Why this matters:**
Educational mode makes Banking Sentinel work for client presentations where the audience needs to understand what is happening. Demo mode makes it work for conferences and videos where pace matters more than explanation. Same system. Same intelligence. Two audiences served.
...
entity BUT050 {  // Business Partner Relationships — confirmed table name by bank architects
...
RPT-1 (now RPT-1.5) is SAP's foundation model for tabular prediction. Called via API — POST request to the SAP Generative AI Hub endpoint. No model training required. Any language, any HTTP client.
...
**Where it fits:** Pattern Agent calls RPT-1 via API POST to SAP Generative AI Hub. Gets a statistically grounded risk score. Passes this to LangGraph state. LLM agents reason OVER this score — they do not replace it.
...
AI-native. SAP AI Golden Path recommended. General availability since Q1 2025. Enable via SAP HANA Cloud Central cockpit — no separate installation needed. Uses SPARQL — not standard SQL graph queries. Documentation: help.sap.com/docs/hana-cloud-database/sap-hana-cloud-sap-hana-database-knowledge-graph-guide

**SPARQL query example:**
```sparql
PREFIX banking: <http://banking.sentinel/>

SELECT ?connected ?relType ?hop
WHERE {
  banking:B-001 ?relType ?connected .
  ?connected ?rel2 ?entity .
  FILTER(?hop <= 8)
}
```
...
**What goes in:**
```
Start: B-001, Tables: BUT050 + BCA_GUARANTOR, Depth: 8 hops
Filters: relationship types representing guarantor, parent, trust connections
Note: Actual BUT050 relationship type code values confirmed during implementation
from TRBK data dictionary — purpose of field matters, not the specific code value
```
...
User query: "Analyse B-001"
        |
        v
RPT-1 API call (POST to Generative AI Hub /predict)
...
- G-001 connected via BUT050 to G-002 (same family trust structure) — HIDDEN
- G-002 guarantees B-004 and B-005
- Full group exposure including G-002 network: $7.8M against $8.5M group limit — 92%
- Requires 6-hop graph traversal: B-001 → BKKN → BCA_LOAN_HDR → BCA_GUARANTOR → G-001 → BUT050 → G-002
...
5. What is the nature of the relationship between G-001 and G-002? → Common family trust structure — BUT050 RELTYP = TRUST_COMMON
...
SAP: HANA Knowledge Graph Engine traversal across BUT050 and BCA_GUARANTOR edges using CDS queries with relationship joins.
...
Banking: Agent thinks "something is wrong with B-001," queries DFKKOP, observes 61 days overdue, thinks "check guarantor," queries BUT050, observes TrustCo Holdings, thinks "calculate group exposure."
...
| BUT050 | BP-to-BP Relationships | Connected parties, guarantors, subsidiaries — confirmed by bank architects | **Graph edge 1** |
...
- HANA Knowledge Graph Engine: BUT050 edges + BCA_GUARANTOR edges — for multi-hop traversal
...
| 1 | Synthetic TRBK data CSV files generated · HANA Cloud schema deployed (all tables in Part 9) · Four hidden patterns seeded · Graph relationships loaded in BUT050 and BCA_GUARANTOR · HANA Knowledge Graph Engine verified | Data foundation. HANA Graph topology. | 6-hop traversal finds TrustCo Holdings |
...
**Official APRA document sources (used for live Twinkle 2 demo):**
- APS 221: `https://handbook.apra.gov.au/standard/aps-221`
- CPS 230: `https://www.apra.gov.au/sites/default/files/2023-07/Prudential%20Standard%20CPS%20230%20Operational%20Risk%20Management%20-%20clean.pdf`
- All ADI standards: `https://www.apra.gov.au/industries/1/standards`
- Operational Risk: `https://www.apra.gov.au/operational-risk-management`
...
**Blog Title:** "Connected Intelligence — Agentic AI on SAP BTP with Knowledge Graph, LangGraph and Claude"
...
Four confirmed scenarios for the prototype demonstration. A fifth scenario (autonomous portfolio run) is on hold — not confirmed for this prototype.

### Scenario 1 — Full Risk Analysis (Twinkle 1 lives here)
**Input:** "Analyse borrower B-001 for all risk dimensions"
**What happens:**
- Intake Agent parses intent → routes to risk analysis
- Pattern Agent: RPT-1 scores HIGH (74), PAL detects payment anomaly
- Relationship Agent: SPARQL traverses BUT050 + BCA_GUARANTOR, finds TrustCo Holdings at hop 6 (nobody asked about this)
- Self-RAG fires: confidence 64% → re-queries → confidence 89% (Twinkle 1)
- Trajectory Agent: resolves conflicting signals, calculates future DTI 9.2, time-to-breach 12 days
- Human-in-the-loop interrupt: risk officer approves
- Synthesis Agent: retrieves APRA documents, generates risk brief
**What audience sees:** AI finding a hidden risk chain that was genuinely undiscovered. Self-RAG making a real decision to go back for more data.
...
**What happens:**
- Intake Agent detects simple data query
- Routes directly to simpleQuery node — no full pipeline
- HANA relational query returns: $47.3M across 60 loans
- Offers: "Would you like a risk analysis of any specific borrower?"
**What audience sees:** System handles simple questions cleanly without over-engineering. Not every question needs five agents.

---

### Scenario 3 — Live Regulatory Update (Twinkle 2 lives here)
**Input:** Click "Sync Latest APRA Standards" button, then "Analyse B-001"
**What happens:**
- System connects to live APRA website
- Fetches official documents from handbook.apra.gov.au/standard/aps-221 and CPS 230 PDF
- Chunks, embeds, stores in HANA Vector
- "APRA standards updated. Re-running B-001..."
- Synthesis Agent now retrieves NEW regulatory content
- Risk assessment changes — finding that was not there before the sync now appears
**What audience sees:** Regulation changed in seconds with zero code change. Every bank person in the room has spent months implementing regulatory changes in their core systems. This takes seconds.

---

### Scenario 4 — Deliberate Rejection (Responsible AI)
**Input:** "Approve the loan for B-001"
**What happens:**
- Intake Agent detects action keyword "Approve"
- Routes to rejection node immediately
- Returns standard refusal message
- Displays B-001 risk profile below the refusal
**Exact output:**
"I am a risk intelligence system. I surface findings and recommendations. Loan approval decisions require human authorisation. Here is the risk profile for B-001 to inform your decision."
**What audience sees:** The system knows its boundary. It refuses clearly. It still helps. APRA's co-pilot requirement made visible. Responsible AI in action.

---

### UI Mode Toggle
Each scenario can be run in two modes:
- **Educational Mode:** Popup before each agent explaining purpose, trigger reason, technology, and what it is looking for. Ideal for client presentations.
- **Demo Mode:** No popups. Clean continuous run. Ideal for conferences and video recording.
Toggle button in UI header switches between modes.
...
- Use SQLite — HANA Cloud from day one, no SQLite fallback
- Use BP2000 — the correct table is BUT050, confirmed by bank architects
- Call RPT-1 via SQL — it is an API call to SAP Generative AI Hub /predict endpoint
- Run all five agents for low risk customers — use conditional routing, skip Relationship and Trajectory for low risk
- Add Python to the main application — Python is for RAGAS evaluation scripts only, TypeScript for everything else
...
- Include TRBK table names in all evidence trails — use BUT050 not BP2000
- Make agent reasoning visible in the UI — not just the output
- Keep all five angles in equal consideration
- Implement both Educational Mode and Demo Mode in the UI — toggle button required
- Use SPARQL for HANA Knowledge Graph Engine queries — not standard graph SQL
- Wire the "Sync Latest APRA Standards" button to fetch from live APRA URLs
...
SAP: API call to SAP Generative AI Hub endpoint — POST /predict with feature table. Available via Generative AI Hub trial. RPT-1.5 GA in H2 2026, current version available now.
...
SAP: PAL (Predictive Analytics Library) built into HANA Cloud. Called via SQLScript stored procedure from CAP. Language-agnostic — CAP calls HANA, HANA runs PAL internally. Enabled by AFL__SYS_AFL_AFLPAL_EXECUTE permission on the HDI container.
