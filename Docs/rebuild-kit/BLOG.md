...
A borrower's risk doesn't live in one place. It's spread across their payment behaviour, where their finances are heading, who they're financially connected to, and whether there's actually enough evidence to act on any of it. Miss one of those and a loss event can slip through a credit file that otherwise looks fine. Banking Sentinel checks all four, each step building on what the last one found, and turns the result into a single risk brief a human can review in under two minutes. That's the part that matters regardless of who's reading this: a faster, more complete picture of risk than a single scorecard number can give.

For anyone evaluating the SAP side of it: this is a mixed-stack system with an explicit, deliberate boundary, not a claim that everything here is SAP-native. The data and ML layer is SAP and proven: SAP HANA Cloud for data and vectors, SAP CAP for the service layer, SAP RPT-1 for tabular AI scoring, all interchangeable with HANA PAL and HANA Knowledge Graph Engine in production (see §11). The reasoning and orchestration layer (the LLM, LangGraph, Langfuse) is not SAP, and that's worth saying plainly: AI Core and AI Launchpad simply aren't available on the BTP trial tier this prototype runs on. That's an access constraint, not a finding that the SAP stack is insufficient. Knowing exactly where that boundary sits is more useful to a technical evaluator than a claim that there isn't one.

For anyone evaluating the AI side of it: the architecture is a working example of four patterns worth studying on their own terms, each with a specific job rather than being added for show. A ReAct tool-use loop drives the graph traversal. A Reflexion-style critic (Reflection) checks evidence quality before anything is finalized. Human-in-the-Loop interrupts enforce compliance sign-off. And a RAGAS-inspired claim-source overlap check (cosine similarity, not the RAGAS library) catches an LLM asserting something the retrieved evidence doesn't actually support, which is exactly what happens in the worked example in §6 and §7.
